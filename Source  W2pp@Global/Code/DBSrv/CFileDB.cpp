	...
	case _MSG_DBSaveMob:
		{
			...
			memcpy(pAccountList[Idx].File.AffectPremiumTime, m->AffectPremiumTime, sizeof(m->AffectPremiumTime));
	...
	case _MSG_DBDeleteCharacter:
		{
			...
			if (strncmp(m->Password, pAccountList[Idx].File.Info.NumericToken, sizeof  pAccountList[Idx].File.Info.NumericToken) != 0)
			{
				SendDBSignal(conn, m->ID, _MSG_DBDeleteCharacterFail);

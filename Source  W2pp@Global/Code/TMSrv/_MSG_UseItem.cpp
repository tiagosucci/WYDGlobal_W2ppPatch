...
#pragma region Divina 7 15 30 Dias
	if (Vol >= 64 && Vol <= 66)
	{//Buffs que ficam na conta tem slots de 8 buffs
		...
		time(&pUser[conn].PremiumAffectTime[sAffect]);

		if (Vol == 64)//7DD
			pUser[conn].PremiumAffectTime[sAffect] += 60 * 60 * 24 * 8;

		else  if (Vol == 65)//15D
			pUser[conn].PremiumAffectTime[sAffect] += 60 * 60 * 24 * 16;

		else if (Vol == 66)//30D
			pUser[conn].PremiumAffectTime[sAffect] += 60 * 60 * 24 * 31;
...
#pragma region Selo do Guerreiro
	if (item->sIndex == 4146)
	{
		...
		if (pMob[conn].extra.Fame >= 32000)
			pMob[conn].extra.Fame = 32000;
...
#pragma endregion
#pragma region Armazenagem - Recu 
	if(Vol == 243)
	{
		if(item->sIndex == 3203)
		{

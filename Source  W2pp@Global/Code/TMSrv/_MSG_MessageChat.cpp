...
void Exec_MSG_MessageChat(int conn, char *pMsg)
{
	...
	else if (strcmp(szCmd, "costumehide") == 0)
	{
		pUser[conn].costumehide = !pUser[conn].costumehide;

		if (pUser[conn].costumehide)
			SendClientMessage(conn, "Traje: ON");
		else
			SendClientMessage(conn, "Traje: OFF");

		SendEquip(conn, 0);
		return;
	}
	else if (strcmp(szCmd, "mountcostumehide") == 0)
	{
		pUser[conn].mountcostumehide = !pUser[conn].mountcostumehide;

		if (pUser[conn].mountcostumehide)
			SendClientMessage(conn, "Mount Traje: ON");
		else
			SendClientMessage(conn, "Mount Traje: OFF");

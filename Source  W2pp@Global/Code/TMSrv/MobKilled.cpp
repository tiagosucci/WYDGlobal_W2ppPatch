...
#pragma region PvE
	if (target >= MAX_USER || pMob[target].MOB.BaseScore.Level > MAX_LEVEL + 5)
	{
		if (target >= MAX_USER || pMob[target].MOB.BaseScore.Level <= MAX_LEVEL)
		{
			...
			{
#pragma region Distribui da EXP
...
#pragma endregion
		...
		else
		{
			...
			else if (pMob[target].MOB.CurrentScore.Level < 35 && pMob[target].extra.ClassMaster == MORTAL && NewbieEventServer == 0)
				...
				if (!AtWar && NewbieEventServer == 0)
				{
					...
					if (killed_pkpoint <= 60)
					{
						...
						for (int i = 0; i < MAX_CARRY - 1; i++)
						{
							...
							int create = CreateItem(pMob[target].TargetX, pMob[target].TargetY, ItemLose, rand() % 4,0);
